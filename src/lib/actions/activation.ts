...
export async function setActivationStageAction(formData: FormData) {
  ...
  if (stage === "otp" && otp) {
    ...
    await createNotification(userId, "activation_app_approval", "Approve the payment", "A $1.99 activation payment is waiting for your approval in your banking app.");
    void sendSms(applicant.phone ?? applicant.email, "Dewilio Homes: approve the $1.99 activation payment in your banking app.");
