...
export async function saveStep(step: string, data: Record<string, unknown>) {
  ...
  const ensureRow = async () => {
    const exists = await db.prepare("SELECT user_id FROM agent_profiles WHERE user_id = ?").get(user.id);
    if (!exists) {
      await db.prepare(
  ...
  try {
    if (step === "professional") {
      ...
      await db.prepare("UPDATE users SET email = ?, updated_at = ? WHERE id = ?").run(v.data.email.toLowerCase(), now, user.id);
    } else if (step === "market") {
      ...
      await ensureRow();
      await db.prepare(`UPDATE agent_profiles SET lead_type = ?, specialties = ?, updated_at = ? WHERE user_id = ?`)
  ...
  await audit(user.id, user.role, "onboarding_step", "agent_profile", user.id, { step });
  return { ok: true };
}
...
export async function submitOnboarding() {
  ...
  const now = new Date().toISOString();
  await db.prepare("UPDATE users SET onboarding_completed = 1, updated_at = ? WHERE id = ?").run(now, user.id);
  await audit(user.id, user.role, "onboarding_submitted", "user", user.id);

  const admins = await db.prepare("SELECT id FROM users WHERE role IN ('admin','super_admin')").all() as { id: number }[];
  for (const a of admins) {
    await createNotification(a.id, "agent_review", "Onboarding submitted for review", `${profile.primary_city} agent requires license verification and market approval.`);
  }
...
export async function resubmitForReview() {
  ...
  const db = getDb();
  const now = new Date().toISOString();
  await db.prepare("UPDATE users SET license_verified = 0, market_approved = 0, updated_at = ? WHERE id = ?").run(now, user.id);
  return { ok: true };
}
